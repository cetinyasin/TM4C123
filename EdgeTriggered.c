...
{
	PortF_Init(); // PF4 configured as digital input
	...
	GPIO_PORTF_IBE_R &= ~0x10; // Interrupt generation will be controlled by GPIOIEV
	// Clear IEV(Interrupt Event) 
	GPIO_PORTF_IEV_R &= ~0x10; // Falling edge triggers an interrupt.
	// Set IM(Interrupt Mask)
	GPIO_PORTF_IM_R |= 0x10; // The interrupt is sent to the interrupt controller.(arm interrupt)
	// Clear Flag
...
volatile unsigned long cnt_Int;
...
// debug cnt on watch windows or on led(for every 3x cnt)
int main(void)
{
	EdgeInt_Init();
	while(1)
